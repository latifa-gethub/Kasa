  ...
  return (
    <div className="containerSlider">
      ...
      {tabPictures.map((element, index) => {
        const classApparaitre=index===currentPicture ? `active-slider` : `unactive-slider` ;
      ...
      <span>{currentPicture+1 +"/"+ nbPictures}</span>
       <button className="btn-precedent" onClick={prevSlide}>{"<"}</button>
      <button className="btn-suivant" onClick={nextSlide}>{">"}</button>
    </div>
  );}else{
    return(
      <div className="containerSlider">
